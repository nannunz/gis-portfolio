...
New York’s residents are deeply stratified on the basis of race and class, and evictions for the city’s most vulnerable are not uncommon. Unfortunately, evictions and a lack of access to stable housing can result in a ripple effect through other aspects of a person’s life, resulting in job loss, food insecurity, or poor educational outcomes. I ultimately decided to develop a housing vulnerability prediction model for the Legal Aid Society, an organization in New York City that offers pro bono legal services to low-income New Yorkers across all five boroughs. This model was developed in ArcGIS Pro because the Legal Aid Society is [relatively well resourced](https://www.causeiq.com/organizations/the-legal-aid-society,135562265/). 

The goal was to build a model that could predict which ZIP codes contained households that were at the greatest risk for eviction. The independent variables were to include: race, household income, and proximity to other evictions; the dependent variable would have been the likelihood of an eviction filing. In order to present the data, I hoped to create a storyboard highlighting the highest-risk neighborhoods and key demographic information. This model would hopefully be used by Legal Aid to improve their canvassing and outreach efforts in high-risk neighborhoods. Those facing eviction are frequently low-income people of color, oftentimes with Limited English Proficiency – access to adequate legal representation allows these individuals to have a fighting chance. According to the [Center for American Progress](https://www.americanprogress.org/article/right-counsel-right-fighting-chance/), in most eviction cases, 90 percent of landlords have legal representation, as opposed to only 10 percent of tenants. Providing free legal services helps level the playing field for those who may lack access to the necessary funding or resources to protect themselves in court. 
...
Rather than use a proximity based analysis, I ultimately decided to use the proportion of evictions to residents by ZIP code as the model's location-based variable. 
...
As mentioned at the beginning of this report, the model does run, but with a few errors. This is likely due to the relatively small sample size available: I ultimately only had around 300 rows of data to feed the model, which severely limits its training ability. Nevertheless, I ran the model using 10,000 trees to see if we could glean any preliminary findings from what was available. 
